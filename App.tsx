          ...
          {result ? (
            <div className="animate-in fade-in slide-in-from-bottom-5 duration-700 space-y-12">
              ...
              <section className="bg-slate-900/60 border border-white/5 rounded-[3.5rem] p-10 space-y-8 shadow-2xl">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {result.implementation.preciseScenarios.map((s, i) => (
                    <div key={i} className="group bg-slate-800/40 p-8 rounded-[2.5rem] border border-white/5 hover:border-cyan-500/30 transition-all shadow-lg">
                      <div className="flex items-center gap-3 mb-4">
                        <span className="bg-cyan-500/20 text-cyan-400 px-3 py-1 rounded-lg font-black text-[10px]">场景分析 {i+1}</span>
                        <h4 className="text-lg font-black text-white group-hover:text-cyan-400 transition-colors">{s.scenario}</h4>
                      </div>
                      <p className="text-sm text-gray-400 leading-relaxed font-medium pl-2 border-l-2 border-slate-700">
                        <span className="text-cyan-500/80 font-black text-[10px] block mb-1">解决的深度痛点</span>
                        {s.problemSolved}
                      </p>
                    </div>
                  ))}
              ...
              <section className="bg-slate-900/60 border border-white/5 rounded-[3.5rem] p-10 space-y-10 shadow-2xl relative">
                ...
                <div className="space-y-6">
                  {result.implementation.technicalBreakdown.map((item, i) => (
                    <div key={i} className="group grid grid-cols-1 lg:grid-cols-12 gap-1 bg-slate-800/30 rounded-[2.5rem] overflow-hidden border border-white/5 hover:border-red-500/30 transition-all shadow-xl">
                      <div className="lg:col-span-3 bg-slate-800/60 p-8 flex flex-col justify-center border-r border-white/5">
                        ...
                        <h4 className="text-base font-black text-white group-hover:text-red-400 transition-colors">{item.component}</h4>
                      </div>
              ...
              <section className="bg-gradient-to-br from-slate-900 via-slate-900 to-cyan-900/20 border border-cyan-500/30 rounded-[4rem] p-12 space-y-12 shadow-2xl relative overflow-hidden">
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                    {result.topicPivots.map((pivot, i) => (
                      <div key={i} className="group bg-slate-800/60 p-10 rounded-[3rem] border border-white/5 hover:border-cyan-500/50 transition-all hover:bg-slate-800/90 shadow-2xl">
                         <div className="flex justify-between items-start mb-6">
                            <span className="text-[10px] bg-cyan-500/20 text-cyan-400 px-4 py-1.5 rounded-full font-black uppercase tracking-widest tracking-widest">建议方案 {i+1}</span>
                         </div>
              ...
              <div className="text-center py-10 opacity-40">
                <p className="text-xs font-bold text-gray-500 tracking-widest uppercase">本项目报告基于 Gemini 3 Pro 深度研判逻辑生成 • 仅供中国国际大学生创新大赛申报参考</p>
              </div>
            ...
            <div className="h-full min-h-[800px] flex flex-col items-center justify-center bg-slate-900/20 border-2 border-dashed border-white/5 rounded-[5rem] p-20 text-center opacity-70 backdrop-blur-md relative overflow-hidden">
              ...
              <p className="text-gray-400 max-w-xl font-bold text-base leading-relaxed relative z-10">
                录入您的选题信息（含技术与场景细节）。系统将为您深度对标2025年最新评审趋势，提供包含技术外科手术级诊断、商业盈利闭环分析及选题魔改方案在内的全维度专家研判报告。
              </p>
